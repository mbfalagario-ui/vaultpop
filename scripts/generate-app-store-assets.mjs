  ...
  return rows
    .flatMap((row, rowIndex) =>
      row.map((typeIndex, columnIndex) => {
        ...
        return [
          `<rect x="${tileX}" y="${tileY}" width="${tileSize}" height="${tileSize}" rx="22" fill="${accent}${isHighlighted ? "55" : "22"}" stroke="${accent}" stroke-width="${isHighlighted ? 7 : 3}"/>`,
          text(tileX + tileSize / 2, tileY + 70, labels[typeIndex], {
            color: accent,
            size: 39,
            weight: 700,
            anchor: "middle"
...
function homeScene() {
  const content = [
    appHeader("VaultPop", "Pop Coins. Complete the Chain.", "Premium Dark Arcade"),
    ...
    metric(664, 1160, 440, "Fictional Points", "42", colors.emerald),
...
function cosmeticsScene() {
  const content = [
    ...
    text(164, 910, "Fictional Points", { color: colors.textSecondary, size: 29 }),
