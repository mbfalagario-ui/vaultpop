...
The reviewer account begins with 1,000 Vault Coins, 5 Bonus Lives, 5 Chain Boosts, and 3 Vault Bursts. The owner admin account and `/admin` interface are owner-only and are not required for gameplay review.

VaultPop includes optional in-app purchases for fictional in-game Vault Coins, fixed booster packs, a permanent ad-free upgrade, and an optional monthly VaultPass Plus subscription. Vault Coins buy fixed boosters. A Bonus Life adds 15 seconds, a Chain Boost raises the active combo by 2x, and a Vault Burst immediately opens the Vault and refreshes the board. Vault Coins and boosters are used only inside the arcade puzzle game, have no real-world value, cannot be exchanged, withdrawn, transferred, or converted into cryptocurrency. The app does not include real cryptocurrency, mining, wallets, trading, staking, NFTs, cash-out, gambling, sweepstakes, or real-world rewards.

Products:

- `app.vaultpop.coins.small`: fixed 1,000 Vault Coins for in-game boosters.
- `app.vaultpop.coins.medium`: fixed 5,500 Vault Coins for in-game boosters.
